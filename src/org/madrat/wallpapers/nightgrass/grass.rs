...
#define MOON_Y_OFFSET (0.02f)
#define MOON_Y_OFFSET_SIZE (0.10f)

#define MOON_Y_FACTOR (0.25)

#define MOON_X_MIN		(0.5f)
#define MOON_X_MIN_SIZE	(-0.25f)

#define MOON_X_MAX		(1.0f)
#define MOON_X_MAX_SIZE	(-0.90f)
...
int 		gAnimationRate 		= 0;
int64_t 	gAnimationStopTime 	= 0;
...
static void drawNight(float brightness) {
	rsgProgramFragmentConstantColor(gPFBackground, 1.0f, 1.0f, 1.0f, brightness);
    rsgBindTexture(gPFBackground, 0, gTNight);    
    rsgDrawRect(0.0f, 0.0f, gWidth, gHeight, 0.0f);
}

static void drawMoon(float brightness, float offset)
{
	rsgProgramFragmentConstantColor(gPFMoon, 1.0f, 1.0f, 1.0f, brightness);
    rsgBindTexture(gPFMoon, 0, gTMoon);
        
    float sz = (gWidth>gHeight)?(gHeight/2):(gWidth/2);
    
    //Height FIX - no more then 1/4 of area
    if (sz >= MOON_Y_FACTOR * gHeight)
    	sz = gHeight * MOON_Y_FACTOR;
    
    //Lets calculate it
    const float offs = 1 -  offset;
    const float minX = MOON_X_MIN*gWidth + MOON_X_MIN_SIZE*sz;
    const float maxX = MOON_X_MAX*gWidth + MOON_X_MAX_SIZE*sz; 

    const float x = minX + (maxX-minX)*offs;
    const float y = MOON_Y_OFFSET*gHeight + MOON_Y_OFFSET_SIZE*sz;
   
    rsgDrawRect(x, y, x+sz, y+sz, 0.0f);
}
...
int root(void)
{
    ...
    float PixXOffset = mix((float)gWidth, 0.f, gXOffset);

    rsgBindProgramFragment(gPFMoon);
    drawMoon(0.75f, gXOffset);
    
    rsgBindProgramFragment(gPFGrass);
    drawBlades(0.0f, PixXOffset);

	// stop animation only if the gAnimationStopTime is not 0 and less then time
	// => don't stop if  0 == gAnimationStopTime OR greate-or-equ time
	return (gAnimationStopTime == 0) || (gAnimationStopTime >= rsUptimeMillis())?gAnimationRate:0;
}
